...
import { MapPin, AlertTriangle, Play, RefreshCw, FolderSearch, FolderOpen, Save, FileJson, Archive, Terminal, CheckCircle, Search, Download, Trash2, X, ChevronDown, ChevronRight } from 'lucide-react';
...
const PointProcessor = () => {
  ...
  const [bulkReplace, setBulkReplace] = useState(null);

  // Progress state
  const [progress, setProgress] = useState({ status: "idle", total: 0, processed: 0, current_folder: "", percentage: 0, logs: [] });
  const logsEndRef = useRef(null);

  // Path configurations (point tool keeps its own files)
  const [inputPath, setInputPath] = useState("d:\\Systems\\Point_Files");
  const [outputGpkg, setOutputGpkg] = useState("d:\\\\Systems\\\\MapViewer\\\\Point_Database.gpkg");
  const [doneDir, setDoneDir] = useState("d:\\\\Systems\\\\MapViewer\\\\Point_Done");
  const [conflictsFile, setConflictsFile] = useState("d:\\\\Systems\\\\MapViewer\\\\point_conflicts.json");
  const POINT_TABLE = "mudryia";
  ...
  const handleBrowse = async (type, title, defaultExt, setter) => {
    try {
      const res = await axios.get('/api/v1/point/browse', {
  ...
  const dismissConflict = (c) => conflictOp(
    () => axios.delete('/api/v1/point/conflicts', {
  ...
  const deleteFolder = (folder) => askConfirm({
    ...
    onConfirm: () => conflictOp(
      () => axios.delete('/api/v1/point/folder', {
  ...
  const dismissGroup = (reason, count) => askConfirm({
    ...
    danger: false,
    onConfirm: () => conflictOp(
      () => axios.post('/api/v1/point/conflicts/clear', { reason }, { params: { conflicts_file: conflictsFile } }),
  ...
  const runDeleteGroupFolders = async (items) => {
    ...
    try {
      for (const c of items) {
        await axios.delete('/api/v1/point/folder', {
  ...
  const dismissAllConflicts = () => askConfirm({
    ...
    danger: true,
    onConfirm: () => conflictOp(
      () => axios.post('/api/v1/point/conflicts/clear', {}, { params: { conflicts_file: conflictsFile } }),
  ...
  const replaceDuplicate = (c) => askConfirm({
    ...
    danger: true,
    onConfirm: () => conflictOp(
      () => axios.post('/api/v1/point/duplicates/resolve', {
  ...
  const runReplaceAll = async (group) => {
    ...
    for (let i = 0; i < group.items.length; i++) {
      ...
      try {
        await axios.post('/api/v1/point/duplicates/resolve', {
          folder: c.folder, mode: "replace", input_path: inputPath,
          output_gpkg: outputGpkg, done_dir: doneDir, conflicts_file: conflictsFile
        });
      } catch {
  ...
  const doEmptyDatabase = async () => {
    setError(null);
    const res = await axios.post('/api/v1/point/database/clear', {}, { params: { output_gpkg: outputGpkg } });
    const n = res.data.total_removed ?? 0;
    const msg = res.data.emptied ? `Point database emptied (${n} features removed).` : (res.data.message || "Database already empty.");
    setSuccess(msg);
    fetchScan();
    return msg;
  };
  const emptyDatabase = () => askConfirm({
    title: "Empty the point database?",
    message: "All Point rows will be permanently deleted from the point GeoPackage.\nThis cannot be undone.",
  ...
  // Step 4: migrate GPKG -> mudryia PostGIS with per-user audit
  ...
  const fetchMigrate = async () => {
    try {
      const [st, hi] = await Promise.all([
        axios.get("/api/v1/point/migrate/status", { params: { output_gpkg: outputGpkg } }),
        axios.get("/api/v1/point/migrate/history", { params: { limit: 50 } }),
      ]);
  ...
  const runMigrate = () => askConfirm({
    ...
    message: `Migrate processed points into the "${POINT_TABLE}" map layer?\nAlready-migrated requests are skipped, and the work is logged under your username.`,
  ...
  const doRunMigrate = async () => {
    ...
    setError(null);
    try {
      const res = await axios.post("/api/v1/point/migrate/run", { output_gpkg: outputGpkg });
      const m = res.data.migrated || 0;
      const s = res.data.skipped || 0;
      const doneMsg = `Migrated by ${res.data.migrated_by}: ${m} point(s) into ${res.data.table || POINT_TABLE} (skipped: ${s}).`;
      setSuccess(doneMsg);
      fetchMigrate();
      askConfirm({
        title: "Migration completed",
        message: "Empty the point file database now to start a new task?\nThe map data stays untouched. This cannot be undone.",
        confirmLabel: "Empty & new task",
        danger: true,
        onConfirm: async () => {
          try {
            await doEmptyDatabase();
            setSuccess(doneMsg + " Point database emptied — ready for the next task.");
            fetchMigrate();
            setStep(1);
          } catch (err) {
            setError(err.response?.data?.detail || err.message || "Operation failed");
  ...
  const fetchScan = async () => {
    ...
    try {
      const res = await axios.get('/api/v1/point/scan', {
  ...
  const pollProgress = async () => {
    try {
      const res = await axios.get('/api/v1/point/progress');
      ...
      if (res.data.status === "running") {
        ...
        setRunning(false);
        setSuccess("Point pipeline execution completed successfully!");
        fetchScan();
        setPipelineRan(true);
  ...
  const runPipeline = async () => {
    ...
    try {
      ...
      await axios.post('/api/v1/point/run', payload);
      pollProgress();
    } catch (err) {
      setRunning(false);
  ...
  useEffect(() => {
    try {
      const saved = {
        inputPath: localStorage.getItem("pp_inputPath"),
        outputGpkg: localStorage.getItem("pp_outputGpkg"),
        doneDir: localStorage.getItem("pp_doneDir"),
        conflictsFile: localStorage.getItem("pp_conflictsFile"),
      };
  ...
  useEffect(() => {
    try {
      localStorage.setItem("pp_inputPath", inputPath);
      localStorage.setItem("pp_outputGpkg", outputGpkg);
      localStorage.setItem("pp_doneDir", doneDir);
      localStorage.setItem("pp_conflictsFile", conflictsFile);
    } catch { /* storage unavailable */ }
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-4">
              <div>
                <label className="flex items-center text-xs font-semibold text-gray-600 mb-1">
                  <MapPin className="w-3 h-3 mr-1 text-purple-500" /> Point Input Directory
                </label>
                <div className="flex">
                  ...
                  <button onClick={() => handleBrowse('folder', 'Select Point Shapefiles Directory', '', setInputPath)} disabled={running} className="bg-gray-200 hover:bg-gray-300 border border-gray-300 border-l-0 rounded-r px-2 py-1.5 text-gray-700 transition-colors">
              ...
              <div>
                <label className="flex items-center text-xs font-semibold text-gray-600 mb-1">
                  <Save className="w-3 h-3 mr-1 text-emerald-500" /> Output Point GeoPackage
                </label>
                <div className="flex">
                  ...
                  <button onClick={() => handleBrowse('save_file', 'Select Output Point GeoPackage', '.gpkg', setOutputGpkg)} disabled={running} className="bg-gray-200 hover:bg-gray-300 border border-gray-300 border-l-0 rounded-r px-2 py-1.5 text-gray-700 transition-colors">
                    <Search className="w-3 h-3" />
                  </button>
                </div>
              </div>
              ...
              <p className="text-[11px] text-gray-400 leading-snug">
                Each request folder must contain point (.shp) files. Folders without point geometry are reported as conflicts.
                Only the Point layer is produced; nothing is written to the main Land / Eshghalat pipeline.
              </p>
            </div>
            <div className="px-4 py-3 border-t border-gray-100 bg-gray-50 flex justify-between items-center flex-shrink-0">
              <button onClick={emptyDatabase} disabled={loading || running} title="Delete ALL Point rows from the file database. This cannot be undone." className="px-4 py-2 bg-white border border-red-300 text-red-600 hover:bg-red-50 rounded-md text-sm font-medium transition-colors inline-flex items-center shadow-sm disabled:opacity-50">
        ...
        <div className="w-full lg:w-2/3 flex flex-col min-h-0 bg-white rounded-lg shadow-sm border border-gray-200">
          {step === 3 && !running && (
            <div className="flex flex-col items-center justify-center h-full p-8 text-center">
              ...
              <p className="text-sm text-gray-600">Import point folders into the database and archive them.</p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="border border-gray-200 rounded-lg p-3">
                        ...
                        <div className="flex justify-between text-xs py-0.5">
                          ...
                          <span className="font-bold text-gray-900">{migStatus.gpkg_layers && migStatus.gpkg_layers["Point"] != null ? migStatus.gpkg_layers["Point"] : 0}</span>
                        </div>
                      </div>
                      <div className="border border-gray-200 rounded-lg p-3">
                        <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-2">Map database (PostGIS)</p>
                        <div className="flex justify-between text-xs py-0.5">
                          <span className="text-gray-600">{POINT_TABLE}</span>
                          <span className="font-bold text-gray-900">{migStatus.postgis && migStatus.postgis[POINT_TABLE] != null ? migStatus.postgis[POINT_TABLE] : 0}</span>
                        </div>
                      </div>
                    </div>
                    <div>
